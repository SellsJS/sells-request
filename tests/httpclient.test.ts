...
describe("httpClient module", () => {
  ...
  it("should make a GET request", async () => {
    ...
    const res = await httpClient.get<string>("http://api.com", "test");
  ...
  it("should resolve base URL from environment variable", async () => {
    ...
    const res = await httpClient.get<string>("API_URL", "test");
    expect(res?.data).toBe("ok");
    expect(mockedAxios.get).toHaveBeenCalledWith("http://api.com/test", {});
  });
  ...
  it('should make a PUT request', async () => {
    ...
    const res = await httpClient.put<{ updated: boolean }>('http://api.com', 'item/1', { name: 'New' });
    expect(res?.data.updated).toBe(true);
  });

  it('should make a DELETE request', async () => {
    mockedAxios.delete.mockResolvedValue({ data: { deleted: true } });
    const res = await httpClient.delete<{ deleted: boolean }>('http://api.com', 'item/1');
